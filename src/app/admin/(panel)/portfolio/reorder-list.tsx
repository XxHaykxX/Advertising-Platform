...
export type PortfolioRow = {
  ...
  title: string;
  brand: string;
  /** Which locales carry a title — the portfolio page falls back, but a case
   *  missing a language is worth seeing at a glance rather than by opening it. */
  locales: string[];
  metricCount: number;
};
...
/* Portfolio case studies, ordered by drag-and-drop (2026-07-27) — the same
   pattern as the Projects table: rows drag via @dnd-kit, ↑/↓ buttons do the
   ...
   The Sort column and the "Sort order" number field are gone with it: the
   position is what the list shows, not a number to keep in your head. Search
   filters the loaded rows; while it's active reorder is off, because dragging
   within a filtered subset has no unambiguous meaning for a global order. */
export function ReorderablePortfolioTable({ items }: { items: PortfolioRow[] }) {
  ...
  const visible = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((p) => p.title.toLowerCase().includes(q) || p.brand.toLowerCase().includes(q));
  }, [rows, query]);
  ...
  function persist(next: PortfolioRow[]) {
    setShowSaved(false);
      ...
      <div className="mb-2 flex h-5 items-center gap-2 text-sm">
        ...
        {!filtering && !pending && !showSaved && (
          <span className="text-xs text-muted-foreground">
            Drag a row to change the order cases appear in on the site
          </span>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-2">
        <div className="flex gap-1">
          {(["hy", "ru", "en"] as const).map((l) => (
            <span
              key={l}
              title={p.locales.includes(l) ? `${LOCALE_LABEL[l]} filled in` : `${LOCALE_LABEL[l]} missing`}
      ...
      <td className="px-4 py-2 text-muted-foreground">
        {p.metricCount > 0 ? p.metricCount : <span className="text-muted-foreground/50">—</span>}
      </td>
      <td className="px-4 py-2">
