...
40. The budget range on the brand profile no longer flickers when you save. For an instant the field showed the option you had before, then corrected itself. Nothing wrong was ever stored — but the same underlying behaviour had twice left a saved change looking lost, so it has been removed rather than smoothed over. A refused save now also keeps what you typed instead of clearing the form.
