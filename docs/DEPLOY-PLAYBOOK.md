...
Свод уроков деплоя, собранный из живых доков и рабочих записей владельца (те записи держатся вне репозитория — см. `.gitignore`). Каждый пункт со ссылкой на источник —
...
1. **Миграция прод-БД — ДО push, вручную.** Если менялась `schema.prisma`, применить diff на
   ...
   рабочие записи владельца). SQL миграций хранить в `docs/prod-migrations/`.
...
3. **`@types/*`, нужные при билде — держать в `dependencies`, не в `devDependencies`.**
   ...
   случай разобран в рабочих записях владельца).
...
5. **Push ≠ live.** Билд на Hostinger может **молча упасть**, и прод продолжит отдавать
   ...
   (реальный случай: коммит `0bc603e`, см. `MEMORY.md`). Обязательно проверять
