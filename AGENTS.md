...
логотипы каналов-партнёров (`Partner`), видеоотзывы клиентов (`Testimonial`) и финальный
CTA с формой заказа звонка (`CallRequest`). Блоки логотипов и отзывов сами скрываются на
пустых таблицах — их наполняет staff через `/admin/partners` и `/admin/testimonials`.
