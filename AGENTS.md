...
и «Заказать звонок» → **`/contact`**), четыре карточки видов рекламы, «почему мы»,
логотипы каналов-партнёров (`Partner`), видеоотзывы клиентов (`Testimonial`) и семь целей
рекламодателя, которыми страница и заканчивается. Блоки логотипов и отзывов сами скрываются
на пустых таблицах — их наполняет staff через `/admin/partners` и `/admin/testimonials`.

⚠️ **Финальный CTA с формой заказа звонка снят 19.08** (решение владельца): компонент
`home-cta.tsx` и якорь `#callback` остались в репозитории, но со страницы убраны — вернуть
это одна строка в `src/app/page.tsx`. Следствие: **`CallRequest` больше не наполняется**,
и очередь `/admin/call-requests` перестала пополняться. Единственная точка входа заявки —
форма на `/contact`, которая **не пишет в БД**, а шлёт письмо админу
(`notifyContactMessage`). Форма спрашивает **телефон, а не email** — то же поле
`PhoneInput` + `isValidPhone`, что и у снятой формы звонка; в письме CTA — `tel:`.

**Цели рекламодателя** (`src/lib/ad-goals.ts` + `home-goals.tsx`, 19.08) — семь пунктов из
документа Мариам плиткой над CTA, чей заголовок «Достигайте бизнес-целей» их и накрывает.
Сетка `1 / sm:2 / lg:4` колонки, первая цель занимает две и залита `bg-primary` — семь
одинаковых строк (как было до вечера 19.08) читались стеной. 7 = 1 двойная + 6 = 8 ячеек:
это ровно заполняет и 4, и 2 колонки; восьмая цель эту арифметику ломает. На телефоне —
одна колонка: в две армянские слова («նախապատրաստում») шире плитки и вылезают за край.
