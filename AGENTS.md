...
# Каталог и реклама слиты в /ads (2026-08-14)

`/catalog` больше не отдельная страница — она редиректит на `/ads`, сохраняя query
(`?channel=` и т.п.), для старых закладок. `/ads` — общий список всего инвентаря
(проекты + рекламные места), `/ads/<slug>` — список одного канала со своими
фильтрами (девять каналов и их slug — `src/lib/ad-channels.ts`); пустой канал
показывает блок "скоро" вместо панели фильтров. Фильтры больше не семь хардкод-блоков
в компоненте, а реестр `src/lib/catalog/facets.ts`, читающий характеристики канала
из `src/lib/ad-channel-attrs.ts` (тип конструкции, дейпарт, длительность ролика и
т.д.) и генерирующий по фасету на каждый select/multiselect/boolean-атрибут; та же
таблица кормит и форму владельца места. Подробнее и как добавить десятый фильтр —
`docs/ad-channel-filters.md`. Маркетинговые лендинги каналов ("что это" / "что можно
купить") сняты — канал сразу ведёт в свой инвентарь.
