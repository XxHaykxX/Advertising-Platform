...
- [ ] **Мёртвые ключи словаря (трекер #28).** 206 из 1219 ключей не используются нигде;
      крупнейшие группы — `projectForm.*` (36), `account.form.*` (22, весь блок),
      `deepDive.*` (20), `investment.*` (19), `catalog.*` (13), `applyDialog.*` (10),
      `roi.*` (6), `btn.*` (6). **Пачкой не удалять:** часть ключей достаётся динамически
      (`localizeValue`, `labelKey` в `creator-guide.ts`/`form-shared.ts`, `t(\`prefix.${x}\`)`)
      и в литеральном скане не видна — удалить такой значит отправить сырой ключ в UI
      (инцидент `completeness.item.*` от 31.07). Плюс `report.tabs.*` и часть `keyFacts.*`
      осиротели переделкой героя 05–06.08, им надо дать отстояться. Порядок: отсеять
      динамику → показать владельцу список «точно мёртвых» → удалять подтверждённое.
      Перед правкой `src/lib/i18n.ts` — `git pull` (его же правит контент-редактор
      через `/admin/i18n`).
- [ ] **`npm run lint` сломан** (и был сломан до этих сессий): ESLint 9 не находит
      `eslint.config.*`, в репозитории конфиг старого формата.
- [ ] Убедиться при следующем пуше, что сборка `ee42bf7` уехала (отдельно не поднималась,
      влияния нет).
