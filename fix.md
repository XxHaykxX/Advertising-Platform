...
**Статус: НЕ НАЧАТО.** Владелец диктует пункты, по каждому заданы уточняющие вопросы и выбран
вариант. Реализацию не начинать до отдельной команды — собираем весь список, потом делаем и
тестируем за один заход.

Дублируется в сессионном трекинг-листе (задачи #1–#5).
...
- [ ] Сегменты видимости + подписи у обоих контролов
- [ ] Фильтр «только незаполненные» (метки `INCOMPLETE: N` уже есть — `projectCompleteness`/`missingCount`, отобрать по ним нельзя)
- [ ] Фильтр по владельцу (колонка Owner)
- [ ] Кликабельная разбивка счётчиков
...
- [ ] Удалить `ReportTabs` и его подключение
- [ ] Проверить, что сверху не осталось пустого места от липкой полосы
- [ ] Проверить, что `--offer-bar-h` (`page.tsx:241`) по-прежнему считается для нижней полосы
...
- [ ] Найти причину пропажи шеврона у группы жанров
- [ ] Привести все группы к одному виду
...
- [ ] Три карточки с заголовками и пояснениями
- [ ] Переименовать «Browse» → «Из медиатеки»
- [ ] Компонент прогресса под палитру проекта (зоны уже 16:9 — вписывается)
- [ ] Транспорт загрузки под честные проценты (см. ниже)
- [ ] Превью сразу после загрузки: картинка — сама, MP4 — кадр
- [ ] Показывать процент, «сколько из скольких МБ», кнопку отмены
...
- [ ] Компактные сегменты, прижатые влево
- [ ] Метка заполненности у каждого языка
...
- [ ] Убрать вызов `PartnersGrid` и импорт
- [ ] Если других вызовов нет — удалить и сам компонент
- [ ] Сообщить, какие ключи словаря освободились (сам словарь не править — его ведёт Мариам через админку)
...
- [ ] Флаг в общем модуле, не дублировать условие по трём файлам
- [ ] Ссылки в шапке и подвале под флаг
- [ ] `notFound()` в роуте; учесть `alternates.canonical` (page.tsx:13) и упоминание в sitemap/robots
- [ ] Данные Portfolio в БД и файлы страницы не трогать
